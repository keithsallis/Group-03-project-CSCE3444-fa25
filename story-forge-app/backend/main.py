import os
from dotenv import load_dotenv
import json
from fastapi import FastAPI, HTTPException
from fastapi.middleware.cors import CORSMiddleware
from pydantic import BaseModel
from typing import List, Optional
from langchain_google_genai import ChatGoogleGenerativeAI

load_dotenv()
api_key = os.getenv("GOOGLE_API_KEY")

# --- Pydantic Models ---
class Character(BaseModel):
    name: str
    personality: str
    gender: str

class StoryRequest(BaseModel):
    characters: List[Character]
    genre: str
    environment: str
    style: Optional[str] = "Custom"
    prompt: str
    previous_story: Optional[str] = ""

app = FastAPI()

ALLOWED_ORIGINS = os.getenv("ALLOWED_ORIGINS", "http://localhost:5173").split(",")
app.add_middleware(
    CORSMiddleware,
    allow_origins=[o.strip() for o in ALLOWED_ORIGINS if o.strip()],
    allow_origin_regex=r"http://.*:5173|https://.*\.vercel\.app",
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
)

# Ensure context.json exists, or handle it gracefully
try:
    with open("context.json", "r") as f:
        context_data = json.load(f)
except FileNotFoundError:
    print("WARNING: context.json not found. Using empty defaults.")
    context_data = {"genres": {}, "environments": {}, "styles": {}, "rules": {
        "introduction": "", "character_handling": "", "continuity": "", "style": "", "word_limit": ""
    }}

# --- FIX: CHANGED MODEL NAME TO STABLE VERSION ---
llm = ChatGoogleGenerativeAI(
    model= "gemini-2.5-flash", #"gemini-2.0-flash-lite", # Changed from gemini-2.0-flash-lite
    temperature=0.7,
    google_api_key=api_key
)

@app.post("/generate_story")
def generate_story(request: StoryRequest):
    print(f"Received request: {request.prompt[:50]}...") # Debug print

    character_list_str = "\n".join(
        [f"- Name: {c.name} | Gender: {c.gender} | Personality/Traits: {c.personality}" for c in request.characters]
    )

    genre_context = context_data["genres"].get(request.genre, "")
    env_context = context_data["environments"].get(request.environment, "")
    
    # Handle missing styles gracefully
    default_style = context_data.get("styles", {}).get("Custom", "")
    style_context = context_data.get("styles", {}).get(request.style, default_style)
    
    rules = context_data.get("rules", {})

    full_prompt = f"""
{rules.get('introduction', '')}

**Genre:** {request.genre}
{genre_context}

**Environment:** {request.environment}
{env_context}

**Writer Style (TONE):** {request.style}
{style_context}

**Characters:**
{character_list_str}

**Rules:**
- {rules.get('character_handling', '')}
- {rules.get('continuity', '')}
- {rules.get('style', '')}
- {rules.get('word_limit', '')}
- IMPORTANT: Do not treat personality traits (e.g. "Smart") as the character's surname. Use only the name provided in the 'Name' field.

---
**Previous Story:**
"{request.previous_story if request.previous_story else 'The story has not yet begun.'}"

---
**User Prompt:**
"{request.prompt}"

Continue the story now:
"""
    try:
        response = llm.invoke(full_prompt)
        print("Story generated successfully.") # Debug print
        return {"story": response.content}
    except Exception as e:
        print(f"❌ ERROR GENERATING STORY: {e}") # THIS WILL SHOW YOU THE REAL ERROR IN TERMINAL
        raise HTTPException(status_code=500, detail=str(e))

# To run the server, use the command in your terminal:
# uvicorn main:app --reload