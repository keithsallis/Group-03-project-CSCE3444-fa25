...
function PromptInput({ prompt, onPromptChange, onForge }) {
  ...
  return (
    <div className="relative w-full">
      <input
        aria-label="Story prompt input"    // 🆕 adds a readable name for screen readers
        type="text"
        value={prompt}
      ...
      <button
        onClick={onForge}
        className="absolute inset-y-0 right-0 flex items-center justify-center w-12 text-gray-300 hover:text-white transition-colors"
        aria-label="Submit prompt"           // 🆕 adds a readable name for screen readers
>
        <svg
